...
`v2.20.0` 已开发完成。这一版给规则引用收尾：**装包时能逐条挑规则**（没挑的不进项目、
不进规则编译；公共库以后加规则默认还会进来），**一条引用来的规则能「另存为项目规则」**
脱钩成项目专用副本，改它不影响公共库和别的项目。

`v2.17.0`～`v2.19.0` 的内容：项目资产按**文档**和**代码**两条链路分开、
规则改成**引用**公共库那份（不再各存副本）、本机与云端能**定期同步**（`npm run sync`）。

`v2.12.0`～`v2.15.0` 的内容：两个视图与工具栏收敛、技术档案进项目设置、
新项目从公共资产库挑资产、规则包可打包可导入；沉淀回流（提升为公共资产、
沉淀体检）；立项起步包（推荐该装什么、四条立项提示词、导出开发体系包）；
文档与模板的边界理清、补上 Spec 这一层、工程基线能查链路完整性。

变更记录见 [CHANGELOG](CHANGELOG.md)，验收清单见 [v2.20.0](docs/acceptance/v2.20.0.md)，
...
规则引用模型（`v2.18.0` 起，`v2.20.0` 补齐减法）：

- 项目里装的规则包只记一条**引用**，规则正文留在公共资产库；公共库改一处，所有引用它的项目跟着变
- 装包时可以**逐条挑规则**，没挑的不参与这个项目
- 想只改一个项目时用**「另存为项目规则」**脱钩一份，两边从此各走各的
- 规则编译、立项推荐、沉淀体检都认这套引用关系
