...
- [x] 云端用账号 A 完成一次 AI 合并：目标更新、来源进入垃圾箱、生成恢复快照
- [x] 云端恢复合并记录：目标旧内容和来源都正确恢复
- [x] 云端清空垃圾箱：恢复记录一并删除
- [x] 刷新页面后数据保持一致
- [x] 用账号 B 登录，确认看不到账号 A 的提示词、垃圾箱和恢复记录

### 第 4 步：清理任务验证

- [x] 手动触发一次清理：`curl -H "Authorization: Bearer $CRON_SECRET" https://<生产域名>/api/health/db`
- [x] 确认返回正常，且过期的垃圾箱和恢复快照被删除
- [x] 确认 Vercel Cron 已注册，每天 03:00 自动执行一次

### 第 5 步：生产验证与回滚演练

- [x] 在生产环境走一遍核心流程：找到提示词 → 填写变量 → 一键复制
- [x] 在 Vercel Deployments 找到上一个 Ready 版本执行 Promote，确认回滚后服务可用
- [x] 回滚后重新部署最新版本，确认最终状态正确
...
- 回滚到上一个 Ready 版本后，登录、读取提示词、编辑和 AI 采集均正常。
- 演练结束后已切回最新版本，最终状态正确。