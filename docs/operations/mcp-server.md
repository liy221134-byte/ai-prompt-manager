...
| 工具 | 做什么 | 边界 |
| --- | --- | --- |
| `create_asset` | 新建提示词／规则／文档／图谱节点／模板 | 只新增；图谱节点编号重复会被拦下；验收记录写成「文档 + 文档类型＝验收记录」并用 `requirementCode` 挂需求；**发布记录只能在界面上建**（版本、日期、门禁是人工可验证的证据） |
...
**规则读的是公共资产库那份**（v2.18.0 起）：项目里装的是对包的引用，
所以 `list_rules` 给项目列规则时，内容来自公共资产库，改公共库那份会影响所有引用它的项目。
