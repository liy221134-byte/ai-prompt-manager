...
test("优化前快照会迁移成资产版本，并可以回到优化前", () => {
  ...
  try {
    ...
    try {
      ...
      // 回退会新写一条「回退前快照」，标识必须由调用方重新生成。
      assert.notEqual(
        reopened.restorePromptOptimize(
          "prompt-optimize-1",
          "restore-before-version-1",
        ),
        null,
      );
    } finally {
...
test("回退快照复用旧版本标识会被拒绝", () => {
  const context = createContext();

  try {
    context.database.createPrompt(
      createPrompt({
        id: "prompt-optimize-contract",
        title: "优化前标题",
        content: "优化前正文",
      }),
    );
    context.database.commitPromptOptimize({
      ...
      versionId: "optimize-snapshot-1",
    });

    assert.throws(
      () =>
        context.database.restorePromptOptimize(
          "prompt-optimize-contract",
          "optimize-snapshot-1",
        ),
      /version_id/,
    );
...
test("回到优化前每次都会生成新的回退快照标识", async () => {
  const requests = [];
  ...
  for (const request of requests) {
    assert.match(request.body.versionId, /^version-/);
  }

  // 两次回退必须用不同标识，复用优化前快照标识会写库失败。
  assert.notEqual(requests[0].body.versionId, requests[1].body.versionId);
});
