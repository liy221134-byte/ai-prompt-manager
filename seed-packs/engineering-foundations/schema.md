...
- `0.5.0`：新增「引用优先于复制」；「生产故障演练」回填证据并升级可信度。文件格式和字段定义不变。
- `0.4.0`：新增 7 条（三道门、版本边界、设计文档四块、证据只记一处、资产三层、
  沉淀分流、AI 写入边界）；文件格式和字段定义不变。
- `0.3.0`：新增「系统化调试」一条资产（外部来源采集第一条，来自 superpowers）；
  文件格式和字段定义不变。
